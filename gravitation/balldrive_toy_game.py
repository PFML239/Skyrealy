...
class Actor(cocos.sprite.Sprite):
    ...
    def __init__(self, cx, cy, radius, btype, img, vel=None):
        ...
        self.scale = (radius * 3.0) * scale_x / (self.image.width / 2.0)
        self.btype = btype
...
class BackgroundLayer( Layer ):
    def __init__(self):
        ...
        self.img = pyglet.resource.image('back.png')
        self.anchor_x = 0
        self.anchor_y = 0
        self.scale_x = consts["window"]["width"]/self.img.width
        self.scale_y = consts["window"]["height"]/self.img.height
...
class Worldview(cocos.layer.Layer):
    ...
    def __init__(self, fn_show_message=None):
        ...
        self.grav = world['grav']

        # load resources:
        pics = {}
        pics["player"] = pyglet.resource.image('pikaboo.png')
        pics["food"] = pyglet.resource.image('circle6.png')
        pics["wall"] = pyglet.resource.image('enem.png')
        self.pics = pics
    ...
    def generate_random_level(self):
        ...
        for i in range(wall_num):
            s = random.random()
            #удален множитель wall_scale
            r = rPlayer * ( s + (1.0 - s))  # lerp
    ...
    def update(self, dt):
        ...
        newVel += dt * self.grav * eu.Vector2(0.0, -1.0)
        mv = buttons['up']
        if mv != 0:
